...
import { Text, View, StyleSheet, VirtualizedList, Animated, Dimensions } from 'react-native';
...
export default class InfinityList extends Component {
    ...
    _renderItem = ({ index, item }) => {
        ...
        const opacity = this.scrollY.interpolate({
            inputRange,
            outputRange: [.6, 1, 1, 1, .6],
            extrapolate: 'clamp',
        });

        const scale = this.scrollY.interpolate({
            inputRange,
            outputRange: [.8, 1, 1, 1, .8],
            // extrapolate: 'clamp',
        });

        const translateY = this.scrollY.interpolate({
            inputRange,
            outputRange: [-item_height / 3, 0, 0, 0, item_height / 3],
            // extrapolate: 'clamp',
        });

        const rotateX = this.scrollY.interpolate({
            inputRange,
            outputRange: ['-35deg', '-10deg', '0deg', '15deg', '35deg'],
            // extrapolate: 'clamp',
        });
